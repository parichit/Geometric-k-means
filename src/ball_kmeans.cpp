...
int main(int argc, char* argv[]) {
    ...
    MatrixOur BallK_dataset = load_data("/u/parishar/scratch/DATASETS/real_data/experiment_data/comma_seperated_files/Twitter.csv");

    cout << "Algo : Ball_KMeans," << " Clusters: " << num_clusters << ", Thresh: " << thres << endl;
    cout << BallK_dataset.rows() << " " << BallK_dataset.cols() << endl;

    auto t3 = std::chrono::high_resolution_clock::now();
    res = ball_k_means_Ring(BallK_dataset, true, num_clusters, thres, iters, time_limit, 
                "random", 45);
