...
int num_cluster, int seed, string init_type){
    ...
    if (init_type == "random"){
        ...
        shuffle(test_array.data(), size, seed);
...
void get_ball_ranodm_indices(int *arr, size_t size, int seed)
{
    if (size > 1) 
    {
        ...
        for (i = 0; i < size - 1; i++)
        {
          size_t j = i + (size_t)(::unif_rand() * (double)(size - i));
          if (j >= size) j = size - 1;
          int t = arr[j];
          arr[j] = arr[i];
          arr[i] = t;
...
MatrixOur &extracted_data, int data_prop, int num_points, int seed)
{
    ...
    get_ball_ranodm_indices(test_array.data(), size, seed);
...
#ifdef GEOKM_R_BUILD
    (void) random_seed;
    initial_row = (int)(::unif_rand() * (double)dataset_rows);
    if (initial_row >= dataset_rows) initial_row = dataset_rows - 1;
#else
    if (random_seed == -1) {
...
#ifdef GEOKM_R_BUILD
        OurType rand_num = (OurType) ::unif_rand();
#else
